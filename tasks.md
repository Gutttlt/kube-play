...
<details>
  <summary>Отгадка</summary>
  Для этого нужно создать объект NetworkPolicy, в котором и определить, какие поды могут взаимодействовать: https://kubernetes.io/docs/tasks/administer-cluster/declare-network-policy/. По умолчанию всем можно говорить со всеми в пределах кластера. Как только создаётся объект NetworkPolicy, все поды, подпадающие под spec.selector, перестают принимать соединения ото всех, разрешая только то, что подпадает под ingress.from. Отдельно включать не нужно, после создания объекта начинает работать само.
</details>
