...
<details>
  ...
  Автор, видимо, хочет нам сказать, что много слоёв -- это плохо. Но мы с ним не совсем согласимся. Слои в общем случае полезны, без них кеширование не работало бы.
  В Node.js я ни в зуб ногой (хотя что-то приходилось контейнеризовать), поэтому начнём с чтения нодовских рекомендаций для Докера: https://github.com/nodejs/docker-node/blob/main/docs/BestPractices.md
